...
# REMOVED: from torchkit.utils.py_utils import threaded_func
...
class VideoDataset(Dataset):
  ...
  def restrict_subdirs(self, subdirs):
    ...
    for key in to_remove:
      # Find the actual key in self._dir_tree that ends with this class name
      matching_keys = [k for k in self._dir_tree.keys() if key in k]
      if not matching_keys:
        # Fallback to original logic
        full_path = osp.join(self._root_dir, key)
        if full_path in self._dir_tree:
          self._dir_tree.pop(full_path)
        else:
          # Try with forward slashes on Windows
          full_path = full_path.replace("\\", "/")
          if full_path in self._dir_tree:
            self._dir_tree.pop(full_path)
      else:
        for matching_key in matching_keys:
          self._dir_tree.pop(matching_key)
  ...
  def _get_data(self, vid_path):
    ...
    # FIXED: Load images sequentially instead of using threaded_func
    # This prevents PIL/Tkinter threading issues on Windows
    # Since PyTorch's DataLoader already uses multiple workers, 
    # we don't need additional threading here
    frames = []
    for image_path in frame_paths:
      frames.append(load_image(image_path))
