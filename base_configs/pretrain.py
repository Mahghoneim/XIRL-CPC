...
def get_config():
  ...
  config.root_dir = "C:/tmp/xirl/pretrain_runs/"
  ...
  config.data.root = "./data/xmagical/xmagical/"
  ...
  ## CPC loss.
  config.loss.cpc = ml_collections.ConfigDict()
  config.loss.cpc.temperature = 0.1
  config.loss.cpc.prediction_steps = 3  # Predict 1,2,3 steps ahead
  config.loss.cpc.autoregressor_hidden_dim = 128
  config.loss.cpc.use_negative_sampling = True
  config.loss.cpc.num_negatives = 65536  # Queue size
  config.loss.cpc.momentum = 0.999  # For key encoder
